...
透過把每一個 veth* 接口綁定到 docker0 網卡，Docker 建立了一個虛擬子網路。這個子網路由主機與所有 Docker 容器共享。
...
這種方式似乎可以用來作使容器間互通的方案，但是缺點很明顯，為了要使容器間互通，我們必須要在容器內作硬編碼，舉例說就是 A B 兩容器內的應用要互通，那就要在 A 容器內設定 B 容器的 IP 位址與 port，B 容器也要如此。還有一點更為致命，就是 Docker 容器一旦重啟，IP 位址就會改變，這樣一來又要重新進入容器設定。這顯然不是一個好方法。
...
首先我們再次使用 `docker network inspect app` 來看一下 app 網路內部情形：
